...
SerialPortDriver.prototype._send = function(command, value, cb) {
  var to_send = new Buffer(3);
  to_send.writeUInt8(this._copterid, 0);
...
SerialPortDriver.prototype.takeoff = function(cb, type) {
  ...
  this._send(PROTOCOL_CODES.COPTER_BIND, type, function(copterid) {
    if (copterid > 0) {
...
SerialPortDriver.prototype.flip = function(cb) {
  this._send(PROTOCOL_CODES.COPTER_FLIP, 0x01, cb);
};

SerialPortDriver.prototype.led = function(state, cb) {
  this._send(PROTOCOL_CODES.COPTER_LED, state === 'on' ? 0x01 : 0x00, cb);
};

SerialPortDriver.prototype.land = function(cb) {
  this._send(PROTOCOL_CODES.COPTER_LAND, 0x00, cb);
};
